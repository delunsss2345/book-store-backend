...
export class AdminBookVariantController {
  ...
  @Get()
  @RequirePermissions(PermissionCode.BOOK_VARIANT_READ)
  ...
  @Patch('/:variantId')
  @RequirePermissions(PermissionCode.BOOK_VARIANT_UPDATE)
  @ApiBearerAuth('access-token')
  ...
  updatePrice(
    ...
    @Body() body: AdminUpdatePriceVariant,
  ): Promise<BookVariant> {
    return this.adminBookVariantService.updatePriceVariant(
      Number(variantId),
      body,
    );
  }
