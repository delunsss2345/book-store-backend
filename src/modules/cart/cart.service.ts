import { CartMessage } from '@/common';
...
export class CartService {
  ...
  async addCartItem(request: Request, body: AddCartItemRequestDto) {
    ...
    if (actor.authError || !actor.userId) {
      ...
      const guestCart = await this.cartRepository.findByGuestSessionId(
        actor.guestSessionId,
      );
      if (!guestCart) {
        const createdCart = await this.cartRepository.createByGuestSessionId(
          actor.guestSessionId,
        );
        const createdItem =
          await this.cartItemService.createByCartIdAndBookVariantId(
            createdCart.id,
            bookVariantId,
            quantity ?? 1,
          );
      ...
      const existed = guestCart.items.find(
    ...
    if (existedItem) {
      const updatedItem = await this.cartItemService.updateQuantityById(
        existedItem.id,
        existedItem.quantity + (quantity ?? 1),
      );
      return {
        authError: false,
        cartItem: {
  ...
  private resolveActor(request: Request): {
    authError: boolean;
    userId: bigint | null;
