...
} from '@/modules/search/dto/request';
...
export class SearchController {
  ...
  @Get('')
  @ApiOperation({
    summary: 'Semantic search for books using vector similarity',
  })
  ...
  @Get('filter')
  @ApiOperation({
    summary: 'Filter and sort books by category, price range, and sort order',
  })
  @ApiOkResponse({ type: SearchBookItemDto, isArray: true })
  async searchFilter(
  ...
  @ApiOperation({
    summary: 'Reindex all books into the vector store (admin only)',
  })
