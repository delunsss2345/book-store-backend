...
export class UserEventService {
    ...
    findEventTypeByUser(userId: number) {
        return this.userEventRepository.findEventTypeByUserAll(userId);
    }

    findOrderItemsByUserId(userId: number) {
        return this.orderItemRepository.findByUserId(userId);
    }

    createViewBookEvent(userId: number, body: CreateUserEventRequestDto) {
    ...
    createAddToCartEvent(userId: number, body: CreateUserEventRequestDto) {
        return this.userEventRepository.createUserEvent(
            userId,
    ...
    createRemoveFromCartEvent(userId: number, body: CreateUserEventRequestDto) {
        return this.userEventRepository.createUserEvent(
            userId,
    ...
    async getRecommend(
        ...
        const orderIdsMap = orderIds.map(Number);
    ...
    async mergerRecommendVariantAndOrder(variants: Map<string, ScoredObj>, orders: Map<string, ScoredObj>) {
        ...
        const orderIds = [...orders.keys()].map((x) => Number(x));
        ...
        const orderSignals = ordersWithItems.flatMap((orderRow) => {
            ...
            return orderRow.items
                ...
                .map((item) => ({
                    snapshotId: item.bookVariantSnapshotId as number,
                    createdAt:
    ...
    async buildScoreWithRecommendVariant(userId: number) {
    ...
    async buildScoreWithRecommendOrder(userId: number) {
